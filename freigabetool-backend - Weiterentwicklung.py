...
def lplus_lizenzen_abrufen(benutzername,passwort):
    global lizenzdaten,token,benutzername_py, passwort_py,headers
    
    benutzername_py=benutzername
    passwort_py=passwort
    payload = {
    'grant_type': 'password',
    'client_id': config.lplus_client_id,
    'client_secret': config.lplus_client_secret,
...
def excelliste_generieren():
    ...
    if len(excel_download_liste[0])>4:
        with open(f"Freigabeliste.csv", 'w+', newline='') as myfile:
